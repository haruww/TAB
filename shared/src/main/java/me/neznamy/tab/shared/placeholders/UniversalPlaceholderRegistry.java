...
public class UniversalPlaceholderRegistry implements PlaceholderRegistry {
    ...
    public void registerPlaceholders(@NotNull PlaceholderManager manager) {
        ...
        manager.registerServerPlaceholder(TabConstants.Placeholder.ONLINE, 1000, () -> {
            if (TAB.getInstance() instanceof TAB && ((TAB) TAB.getInstance()).getPlatform().getOnlinePlayerCount() >= 0) {
                return ((TAB) TAB.getInstance()).getPlatform().getOnlinePlayerCount();
            }
            return Arrays.stream(TAB.getInstance().getOnlinePlayers()).filter(all -> !all.isVanished()).count();
        });
        // MultiPaper end
        // RandomTeamFights start
        manager.registerServerPlaceholder(TabConstants.Placeholder.RED_TEAM_ONLINE, 1000, () -> {
            if(TAB.getInstance() instanceof TAB) return TAB.getInstance().getPlatform().getRedTeamPlayerCount();
            return 0;
        });
        manager.registerServerPlaceholder(TabConstants.Placeholder.BLUE_TEAM_ONLINE, 1000, () -> {
            if(TAB.getInstance() instanceof TAB) return TAB.getInstance().getPlatform().getBlueTeamPlayerCount();
            return 0;
        });
        manager.registerPlayerPlaceholder(TabConstants.Placeholder.PLAYER_KILL_COUNT, 2000, p -> ((TabPlayer)p).getKillCount());
